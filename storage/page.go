...
// Page 大小常量
const (
	PageSize     = 4096           // 4KB 页大小
	PageHeaderSize = 24           // 页头大小
	MaxPageData  = PageSize - PageHeaderSize
)

// Page 类型
const (
	PageTypeFree       uint8 = 0x00 // 空闲页
	PageTypeMeta       uint8 = 0x01 // 元数据页
	PageTypeCatalog    uint8 = 0x02 // 目录页（Collection/Index 目录）
	PageTypeData       uint8 = 0x03 // 数据页
	PageTypeIndex      uint8 = 0x04 // 索引页
	PageTypeOverflow   uint8 = 0x05 // 溢出页（存储大文档）
	PageTypeFreeList   uint8 = 0x06 // 空闲列表页
)
...
// Slot 表示一个槽
type Slot struct {
	Offset uint16 // 记录在页面中的偏移
	Length uint16 // 记录长度
	Flags  uint16 // 标志（删除标记等）
}

const (
	SlotSize        = 6  // 每个槽占用 6 字节
